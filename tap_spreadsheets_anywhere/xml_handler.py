...
def get_row_iterator(table_spec, reader, xslt_reader=None):
    try:
        dom = etree.parse(reader)
        if xslt_reader:
            transform = etree.parse(xslt_reader)
            transform = etree.XSLT(transform)
            dom = transform(dom)
        res = xmltodict.parse(dom)
        json_array = res['Leads']['LeadData']
